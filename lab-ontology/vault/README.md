...
这是一个面向 AI Agent 决策与执行的知识库骨架。Markdown 文件与 Git 历史是唯一事实源；Native 索引、向量和关系视图都是可从指定 Git 提交重建的派生层。

## 三层结构

| 层 | 位置 | 用途 |
|---|---|---|
| Raw | `.raw/` | 原始材料，只保留可复原性，不进入检索索引 |
...
- Project 等状态页是可持续更新的活页面；任何 Agent 都可提出增量修改，但必须经过精确提案与用户批准。
- 周期性全库审计检查格式、重复、矛盾、过期和断链，默认只报告并形成分项提案，不自动清洗。
- 专用业务系统可以继续作为各自领域的事实源；只有其正式导出才经知识网关进入本库。
- 所有写入先提案、再批准；任何 Agent 都不得直接修改正式知识。
